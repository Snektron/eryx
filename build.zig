...
fn addFutharkSrc(b: *Builder, exe: *std.build.LibExeObjStep, backend: FutharkBackend) void {
    ...
    const futhark_output = std.fs.path.join(b.allocator, &[_][]const u8{ cache_root, "futhark" }) catch unreachable;

    const futhark_gen = b.addSystemCommand(&[_][]const u8{ "futhark", std.meta.tagName(backend), "--library", "src/main.fut", "-o", futhark_output });
